...
const PlayfairChallenge = () => {
  ...
  }, [timeLeft, start, cipher, plaintext, type]);

  const next = () => {
    if (idx + 1 < MAX_PUZZLES) setIdx(i => i + 1);
    else { 
      setStart(false); 
      setFb(`🎉 Done! Score: ${score}/${MAX_PUZZLES}`); 
    }
  };
  ...
  const onSubmit = (e) => {
    ...
    if (correct) {
      ...
      setTries(t);
      if (t >= MAX_ATTEMPTS) {
        const correctAns = type === 'Decrypt' ? plaintext : cipher;
        setFb(`❌ No tries left! Answer: ${correctAns}`);
        clearTimeout(timerRef.current);
        setTimeout(() => next(), 2500);
      } else {
  ...
  const prompt = () => {
    if (type === 'Decrypt')
      return <p>🔐 Decrypt using key <strong>{keyText}</strong>: <strong>{cipher}</strong></p>;
    if (type === 'Encrypt')
      return <p>🔒 Encrypt using key <strong>{keyText}</strong>: <strong>{plaintext}</strong></p>;
